    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '1.25rem', width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 140px), 1fr))', gap: '1rem', width: '100%' }}>
        ...
        <div className="glass-panel" style={{ padding: '1.25rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#06b6d4' }}>
            {stats.totalTenders}
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '1.25rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: '#f59e0b' }}>
        ...
        <div className="glass-panel animate-fade-in" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
          ...
          {reviews.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%' }}>
              {reviews.map((req) => (
                <div key={req.id} style={{ padding: '1rem', borderRadius: 'var(--radius-md)', background: 'rgba(245, 158, 11, 0.05)', border: '1px solid rgba(245, 158, 11, 0.25)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', width: '100%' }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, marginTop: '0.35rem' }} className="text-break">
                      {req.requirementText}
                    </div>
                  </div>
      ...
      {activeAdminTab === 'rules' && (
        <div className="glass-panel animate-fade-in" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%' }}>
            {rules.map((rule) => (
              <div key={rule.id} style={{ padding: '1.1rem', borderRadius: 'var(--radius-md)', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--border-subtle)', width: '100%' }}>
                ...
                <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginTop: '0.35rem' }} className="text-break">{rule.name}</h4>
                <div style={{ fontSize: '0.75rem', color: '#06b6d4', marginTop: '0.15rem' }}>Authority: {rule.authority}</div>
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginTop: '0.5rem' }} className="text-break">{rule.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeAdminTab === 'audit' && (
        <div className="glass-panel animate-fade-in" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
            {auditLogs.map((log) => (
              <div key={log.id} style={{ padding: '0.75rem', borderRadius: 'var(--radius-sm)', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8125rem', flexWrap: 'wrap', gap: '0.5rem', width: '100%' }}>
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }} className="text-break">
